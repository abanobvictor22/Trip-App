...
export function showInteractiveToast(message, type = 'info', actionCallback = null, actionBtnText = '') {
    ...
    let icon = '🔔';
    if (type === 'add') { bgBorder = 'bg-emerald-900/95 border-emerald-500/50 text-white shadow-emerald-900/30'; icon = '✨'; }
    else if (type === 'edit') { bgBorder = 'bg-blue-900/95 border-blue-500/50 text-white shadow-blue-900/30'; icon = '✏️'; }
    else if (type === 'delete') { bgBorder = 'bg-rose-900/95 border-rose-500/50 text-white shadow-rose-900/30'; icon = '🗑️'; }
    ...
    if (actionCallback && actionBtnText) {
        ...
        actBtn.className = 'bg-white/20 hover:bg-white/30 text-white px-3 py-1 rounded-lg text-xs font-black transition cursor-pointer';
        actBtn.innerText = actionBtnText;
        actBtn.onclick = () => { actionCallback(); removeToast(); };
        rightDiv.appendChild(actBtn);
    }

    const closeBtn = document.createElement('button');
    closeBtn.className = 'text-white/60 hover:text-white text-base leading-none font-black transition px-1';
    closeBtn.innerHTML = '✕';
    ...
    function removeToast() {
        ...
        toast.style.transform = state.currentLang === 'ar' ? 'translateX(100%)' : 'translateX(-100%)';
...
export function requestNotificationPermissionUI() {
    if (!('Notification' in window)) {
        alert(t('متصفحك لا يدعم الإشعارات المنبثقة.', 'Your browser does not support push notifications.'));
        return;
    }
    Notification.requestPermission().then(permission => {
        updateNotificationButtonState();
        if (permission === 'granted') {
            showInteractiveToast(t('تم تفعيل التنبيهات المنبثقة بنجاح 🔔', 'Push notifications enabled successfully 🔔'), 'add');
